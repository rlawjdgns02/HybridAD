...
# =========================
# 3. Classical Model (Linear classifier)
# =========================
...
print("CNN frozen - only training classifier")

# =========================
# 5. TRAIN (classifier only)
# =========================
...
# =========================
print(f"\n[Classical] {N_SHOT}-shot Best Accuracy: {best_acc:.4f} (epoch {best_epoch})")
...
torch.save({
    ...
    "history": history
}, f"results/classical_{N_SHOT}shot.pth")
print(f"Model saved to results/classical_{N_SHOT}shot.pth")

# Loss & Accuracy 그래프
fig, axes = plt.subplots(1, 2, figsize=(12, 4))

axes[0].plot(history["loss"], label="Loss", color="blue")
axes[0].set_xlabel("Epoch")
axes[0].set_ylabel("Loss")
axes[0].set_title(f"Classical {N_SHOT}-shot: Loss")
axes[0].legend()
axes[0].grid(True)
...
axes[1].set_title(f"Classical {N_SHOT}-shot: Accuracy (best: {best_acc:.4f} @ ep{best_epoch})")
axes[1].legend()
axes[1].grid(True)

plt.tight_layout()
plt.savefig(f"results/classical_{N_SHOT}shot.png", dpi=150)
plt.show()
print(f"Plot saved to results/classical_{N_SHOT}shot.png")
